...
library(tidyr)
library(tibble)
library(stringr) #to remove dots and replace with spaces
...
#Write a function to wrangle the data
wrangle_data <- function(se_object, input_genes, selectcol_metadata) {
  # 1. Extract expression matrix
  expr_df <- as.data.frame(assay(se_object))
  colnames(expr_df) <- trimws(colnames(expr_df))  # Clean sample names
  ...
  meta_df <- as.data.frame(colData(se_object))
  rownames(meta_df) <- trimws(rownames(meta_df))  # Clean sample IDs
  meta_df$sample_id <- rownames(meta_df)          # Make sample ID a column
  colnames(meta_df) <- str_replace_all(colnames(meta_df), "\\.", " ")
  
  # 3. Convert expression data to long format
  merge_df <- expr_df %>%
    pivot_longer(
      cols = -gene,
      names_to = "ID",
  ...
  # 4. Validate metadata column
  if (!selectcol_metadata %in% colnames(meta_df)) {
    stop(paste0("Column '", selectcol_metadata, "' not found in metadata."))
  }
  
  # 5. Append selected metadata column using match
  matched_idx <- match(merge_df$ID, meta_df$sample_id)
  merge_df[[selectcol_metadata]] <- meta_df[matched_idx, selectcol_metadata]
  
 
  # 6. Return final merged dataframe
  return(merge_df)
}
